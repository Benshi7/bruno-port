...
const CardComponent = ({ title, description, url, github, alt, src, disponible = false }) => {
  return (
      ...
      <div className="flex items-center pl-0">
        ...
        <p className="hidden justify-end items-end text-balance text-sm font-light 2xl:flex">{description}</p>
        <div className='mb-10 lg:mb-0 ml-0 lg:ml-24 2xl:ml-0 '>
        { url !== ''
          ? (<>
            <a target='blank' href={url} className={`${disponible === false ? null : 'hover:text-red-800 transition-colors'} hidden 2xl:block hover:scale-105 transition-all`}>{url || 'Sitio no disponible'}</a>
            <a target='blank' href={url} className='block 2xl:hidden hover:scale-105 transition-all'><MingcuteLinkFill className=' w-8 h-8 ml-12 -mb-6 lg:w-10 lg:h-10 lg:-mb-4 xl:w-12 xl:h-12' /></a>
          </>)
          : <><a target='blank' className='block 2xl:hidden hover:scale-105 transition-all'><MingcuteLinkFill className='w-8 h-8 ml-12 -mb-12 lg:w-10 lg:h-10 lg:-mb-4 opacity-15 xl:w-12 xl:h-12'/></a>
            <p className='hidden 2xl:block hover:scale-105 transition-all'>⌛</p></>
        }

        <a target='blank' href={github} className='hidden 2xl:block hover:scale-[1.03] hover:text-red-800 transition-all'>Github</a>
        <a target='blank' href={github} className='hover:text-red-800 text-right pl-24 transition-all visible 2xl:hidden text-white '><MdiGithub className='w-8 h-8 ml-12 -mb-12 lg:w-10 lg:h-10 lg:-mb-4  xl:w-12 xl:h-12' color='white'/></a>
        </div>
        </div>
        <Card
          isPressable
